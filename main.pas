(*
  v0.9 voor Rad Studio 10.3: win64, win32 en android. 09-01-2018

  woordenlijst:
  http://woordenlijst.org/#/
  namen lijsten:  (not used anymore)
...
uses
    ...
    FMX.Platform, FMX.TextLayout, System.Messaging, System.Notification,
    FMX.ScrollBox, FMX.Memo, FMX.Grid.Style, FMX.SpinBox, FMX.FontGlyphs,
    FMX.Grid, FMX.VirtualKeyboard, FMX.Edit.Style, MyKeyboard, FMX.EditBox, FMX.Media;
...
type
    ...
    CloseButton: TButton;
        ...
        procedure terminateAnaZoekactie(Sender: TObject);
...
{$ENDIF}
    ...
    Bitmap1.LoadFromFile(DocumentsPath+'snail.png');
    Bitmap2 := TBitmap.Create;
    Bitmap2.LoadFromFile(DocumentsPath+'duck.png');
    Choose_animation;
...
procedure TWZmainform.NotifyComplete;
var
    t: TTimeSpan;
begin
    ...
    begin
        ...
        Notification.Name := 'Puzzelhulp';
        t := Stopwatch.Elapsed;
        Notification.AlertBody := IntToStr(ListView1.Items.Count) + ' anagr. "' + ZoekWoord + '" in ' + format('%.2d:%.2d:%.2d',[t.Hours,t.Minutes,t.Seconds]);
...
begin
    if workerthread.FZoekOptie = anagramzoek then
    begin
        Image7.Visible := true;
...
procedure TWZmainform.Choose_animation;
var
...
begin
    ...
    case choice of
        0:
            begin
                BitmapListAnimation1.AnimationBitmap := Bitmap1;
                BitmapListAnimation1.AnimationCount := 12;
                BitmapListAnimation1.Duration := 1.4;
           end;
//        1:
//            begin
//                BitmapListAnimation1.AnimationBitmap := Bitmap2;
//                BitmapListAnimation1.AnimationCount := 20;
//                BitmapListAnimation1.Duration := 1.3;
//           end;
        1:
            begin
                BitmapListAnimation1.AnimationBitmap := Bitmap2;
                BitmapListAnimation1.AnimationCount := 83;
                BitmapListAnimation1.Duration := 9;
           end;
    end;
end;
...
procedure TWZmainform.CloseButtonClick(Sender: TObject);
begin
...
begin
    ...
    begin
        //thread.Terminate;
        if thread is TWorkerThread then
        begin
            (thread as TWorkerThread).StopThread;
        end;
    end;
end;
...
var
    ...
    c: char;
begin
    if Assigned(workerthread) then //and (workerthread.Terminated = false) then
    begin
        StopAndClearClick(nil);
        //exit;
    end;
    ...
    ZoekWoord := orig;
    ZoekOpdrachtCanceled := false;
    workerthread := TWorkerThread.Create(orig, anagramzoek);  //true = anagram search
    workerthread.OnTerminate := terminateAnaZoekactie;
    workerthread.Start;
...
begin
    if workerthread.Terminated = true then
    begin
        ZoekOpdrachtCanceled := true;
        exit;
    end;

    list := doscrabblezoek(diff);
    if not gehad.ContainsKey(diff) then gehad.Add(diff, 0);
    ...
    begin
        ...
        begin
            ...
            if AnaSelect.IsChecked then //and (level > 0) then
            begin
                if gehad.ContainsKey(newdiff) then continue;
            end;
...
procedure TWZmainform.terminateAnaZoekactie(Sender: TObject);
var
    n: integer;
begin
//    AssignToListBox1(ResultList);
...
//    ShowMessage(IntToStr(Stopwatch.ElapsedMilliseconds));
    if CheckNotify.IsChecked and not ZoekOpdrachtCanceled then NotifyComplete;
end;
